...
module.exports = {
	...
	async execute(interaction) {
		...
		if (submitted) {
			...
			for (const [key, data] of submitted.fields.fields.entries()) {
				...
				row.addComponents(
					new ButtonBuilder()
						.setCustomId(`${data.value}-${data.customId}`)
						.setLabel(data.value)
			...
			collector.on('collect', async i => {
				...
				for (const [key, data] of results.entries()) {
					...
					if (interaction.options.getBoolean('show_users')) {
						field_text += ' - '
						for (const userId of results.get(key)?.users.slice(0, 10)) {
							field_text += `<@${userId}>`
						}
					}
				...
				await i.update({embeds: [pollEmbed]})
